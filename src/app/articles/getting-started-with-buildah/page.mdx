...
import buildahContainers from './buildah-containers.png';

export const article = {
  author: 'Cedric Clyburn',
  date: '2022-11-14',
  title: 'Getting started with Buildah',
  description: 'Use Buildah to create a working Open Container Initiative container image from scratch, or from a pre-existing Dockerfile, before running it with Podman.',
}
...
If you're running [Red Hat Enterprise Linux 9](https://developers.redhat.com/products/rhel/overview) (RHEL 9), follow the steps below. For Fedora users, be sure to replace `yum` with `dnf`:

```bash
$ yum -y install buildah
```

For other distributions of Linux, follow these [docs](https://github.com/containers/buildah/blob/main/install.md).
...
![Listing our stored container images](https://developers.redhat.com/sites/default/files/blog/2020/08/Buildah-containers.png)
...
Your `redhat-website` image is ready to [run with Podman](https://developers.redhat.com/blog/2019/08/14/best-practices-for-running-buildah-in-a-container/), or pushed to your registry of choice.

Building with a Dockerfile
--------------------------

Another significant part of Buildah is the ability to [build images using a Dockerfile](https://github.com/containers/buildah/blob/main/docs/buildah-build.1.md), and the `build-using-dockerfile`, or `bud` command can do just that. Let's take an example Dockerfile as input, and output an OCI image:
...
Great job! We've gone through building a container from scratch, as well as from a predefined Dockerfile. Buildah is a lightweight and flexible way to create container images without the need for a runtime or daemon installed. If you need container orchestration, you can use Buildah with [Kubernetes](https://developers.redhat.com/topics/kubernetes)
