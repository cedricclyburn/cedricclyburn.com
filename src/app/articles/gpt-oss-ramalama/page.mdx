...
import ramalamaServeGptOss from './ramalama-serve-gpt-oss.png'
import nvtopGpuMetrics from './nvtop-gpu-metrics.png'
...
The release of [OpenAI's gpt-oss models](https://openai.com/index/introducing-gpt-oss) is a significant milestone for developers and enterprises looking to control their own AI journey. These open-weight models, available in 20B and 120B parameter variants, bring ChatGPT-level reasoning capabilities to your local machine under the [Apache 2.0 license](https://huggingface.co/openai/gpt-oss-120b). But here's the catch: How do you run these models securely, without compromising your system or spending hours configuring GPU drivers?

Enter [RamaLama](https://github.com/containers/ramalama), a command-line tool that makes running AI models as simple as running containers. By leveraging OCI containers and intelligent GPU detection, RamaLama eliminates the complexity of AI infrastructure while providing strong isolation via containerization.
...
Before diving into the setup, let's address the elephant in the room: Why not just use [Ollama](https://ollama.com/) or run the models directly?
...
Both models support a 128,000k token context length (although reduced context lengths between 8k-32k are recommended unless you have 80GB or more of VRAM or substantial unified memory on Apple silicon).

They use MXFP4 quantization, which enables memory-efficient deployment on consumer GPUs. Read [Optimizing generative AI models with quantization](https://developers.redhat.com/articles/2025/08/18/optimizing-generative-ai-models-quantization) to learn more about how quantization works.

Benchmarks show 20B is approximately equivalent to o3-mini, and 120B is approximately equivalent to o4-mini on tasks like reasoning, coding, and MMLU (learn more on the [OpenAI blog](https://openai.com/index/introducing-gpt-oss/)).
...
One line, and that's it. You can now use `ramalama` in the terminal to pull, run, and serve models from your system. Behind the scenes, RamaLama will automatically detect your container runtime, like [Podman](https://podman.io/) or [Docker](https://www.docker.com/), when running and serving models.
...
<Image src={ramalamaServeGptOss} alt="Serving the gpt-oss model locally using RamaLama and testing the local web interface" />
...
Alternatively, we can use `nvtop`, the task monitor for NVIDIA GPUs and other accelerators to monitor load and memory usage:

```bash
nvtop
```

<Image src={nvtopGpuMetrics} alt="GPU usage metrics with nvtop for real-time monitoring of accelerator load" />
...
- Join the [Matrix chat](https://matrix.to/#/#ramalama:fedoraproject.org).
- Try the [quick start examples](https://github.com/containers/ramalama#quickstart).
- Read the blog post [How RamaLama runs AI models in isolation by default](https://developers.redhat.com/articles/2025/02/20/how-ramalama-runs-ai-models-isolation-default).
